...
import equivalencyandcloning.SinglyLinkedList;
...
public class CircularlyLinkedList<E> implements Cloneable{
  ...
  public CircularlyLinkedList<E> clone() throws CloneNotSupportedException {
    // always use inherited Object.clone() to create the initial copy
	  CircularlyLinkedList<E> newList = (CircularlyLinkedList<E>) super.clone(); // safe cast
    
    if (size > 0) {                    // we need independent chain of nodes
    	newList.tail = new Node<>(tail.getElement(), null);
      Node<E> walk = tail.getNext();      // walk through remainder of original list
      Node<E> recent = newList.tail;     // remember most recently created node
      while (walk != tail) {              // make a new node storing same element
        Node<E> newest = new Node<>(walk.getElement(), null);
        recent.setNext(newest);     // link previous node to this one
        recent = newest;
        walk = walk.getNext();
      } 
      recent.setNext(newList.tail);
    }
    return newList;
  }
  /**
...
//main method
  public static void main(String[] args) throws CloneNotSupportedException
  {
	  ...
	  CircularlyLinkedList<String> bCircularList = circularList.clone();
	  
	  circularList.removeFirst();
	  System.out.println(circularList);

	  System.out.println(bCircularList);
